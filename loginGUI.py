...
with open('userlist.csv') as csv_file: #so dont have to do csv_file.close() and also keeps going until done reading file
...
def rd():
    ...
    host = str.encode(socket.gethostname())  # Get local machine name
    s.connect((host, port))
...
userna = ttk.Entry(mainframe, width=20, textvariable=usern)
userna.grid(column=2, row=2, sticky=(W, E))
paswo = ttk.Entry(mainframe, width=20, textvariable=pasw, show="*")
...
ttk.Button(mainframe, text="Login", command = rd).grid(column=2, row=5, sticky=(N,E))
